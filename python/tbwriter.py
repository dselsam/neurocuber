...
class TensorBoardWriter:
    ...
    def log_actor_episode(self, actor_episode_result):
        ...
        value = [
            tf.Summary.Value(tag="n_steps/vs_%s" % aer.brancher, simple_value=aer.n_steps),
            tf.Summary.Value(tag="n_cores/vs_%s" % aer.brancher, simple_value=aer.n_core),
            tf.Summary.Value(tag="2^{n_steps}/vs_%s" % aer.brancher, simple_value=math.pow(2, aer.n_steps)),
            tf.Summary.Value(tag="2^{n_core}/vs_%s" % aer.brancher, simple_value=math.pow(2, aer.n_core)),
