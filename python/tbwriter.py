...
class TensorBoardWriter:
    ...
    def log_actor_episode(self, actor_episode_result):
        ...
        self.writers[aer.cuber].add_summary(
            tf.Summary(value=[
                tf.Summary.Value(tag="vs_%s/n_steps" % aer.brancher, simple_value=aer.n_steps),
                tf.Summary.Value(tag="vs_%s/n_cores" % aer.brancher, simple_value=aer.n_core),
                tf.Summary.Value(tag="vs_%s/2^{n_steps}" % aer.brancher, simple_value=math.pow(2, aer.n_steps)),
                tf.Summary.Value(tag="vs_%s/2^{n_core}" % aer.brancher, simple_value=math.pow(2, aer.n_core)),
                tf.Summary.Value(tag="profile/n_secs_play_vs_%s" % aer.brancher, simple_value=aer.n_secs_play),
                tf.Summary.Value(tag="profile/n_secs_data_vs_%s" % aer.brancher, simple_value=aer.n_secs_data)
            ]),
